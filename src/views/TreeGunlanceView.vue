...
import RAW_DATA from '@/data/weapons/trees/gl.json'
...
<template>
	...
	>
		<template #default="{item, isSearchPath, isFocusPath}">
			<!-- prettier-ignore -->
			<WeaponItemShelling :weapon="(item as unknown as TWeapon<TGunlanceData>)" :dimmed="!isFocusPath && isFilterActive && !matchedIds?.has(item.id) && !isSearchPath" />
		</template>
		<template #filter>
			<WeaponFilterPanel
				show-shelling
