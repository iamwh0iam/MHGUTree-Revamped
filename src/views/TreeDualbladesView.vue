...
import RAW_DATA from '@/data/weapons/trees/db.json'
...
<template>
	...
	>
		<template #default="{item, isSearchPath, isFocusPath}">
			<!-- prettier-ignore -->
			<WeaponItemElem2 :weapon="(item as unknown as TWeapon<TDualbladesData>)" :dimmed="!isFocusPath && isFilterActive && !matchedIds?.has(item.id) && !isSearchPath" />
		</template>
		<template #filter>
