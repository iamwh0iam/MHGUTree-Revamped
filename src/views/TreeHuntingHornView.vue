...
import RAW_DATA from '@/data/weapons/trees/hh.json'
...
<template>
	...
	>
		<template #default="{item, isSearchPath, isFocusPath}">
			<!-- prettier-ignore -->
			<WeaponItemHorn :weapon="(item as unknown as TWeapon<THuntingHornData>)" :dimmed="!isFocusPath && isFilterActive && !matchedIds?.has(item.id) && !isSearchPath" />
		</template>
		<template #filter>
